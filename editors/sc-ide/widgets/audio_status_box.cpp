    ...
    StatusBox(parent)
{
    ...
    // server -> box
    connect( server, SIGNAL(runningStateChanged(bool,QString,int)), this, SLOT(onServerRunningChanged(bool,QString,int)) );
...
{
    auto color = running ? Qt::green : Qt::white;

    for( StatusLabel * label : { mStatisticsLabel, mVolumeLabel } )
